...
app.get('/api/emails', async (req, res) => {
  try {
    ...
    const listRes = await gmail.users.messages.list({ userId: 'me', labelIds: ['INBOX'], q: 'is:unread', maxResults: 1 });
    if (!listRes.data.messages || listRes.data.messages.length === 0) return res.json({ email: null });
...
app.post('/api/send', async (req, res) => {
  try {
    ...
    const gmail = getGmailClient();
    const msgRes = await gmail.users.messages.get({ userId: 'me', id: emailId, format: 'metadata', metadataHeaders: ['From', 'Subject', 'Message-ID'] });
    ...
    const raw = Buffer.from(`To: ${from}\r\nSubject: Re: ${subject}\r\nIn-Reply-To: ${messageId}\r\nReferences: ${messageId}\r\nContent-Type: text/plain; charset=utf-8\r\n\r\n${response}`).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
    await gmail.users.messages.send({ userId: 'me', requestBody: { raw, threadId: msgRes.data.threadId } });
